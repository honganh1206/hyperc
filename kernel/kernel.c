...
int register_syscall() {
  asm(
      ...
      "mov ecx, $[msr_start];" // Set ECX = 0xc0000081 (MSR_STAR ID)
      ...
      // Overflow Most importantly: Clears IF (Interrupt Flag) to disable
      ...
      // Point CPU to kernel handler (syscall_entry assembly function)
      // Specify 64-bit kernel handler access
      // When user code executes SYSCALL, CPU jumps here
      "lea rax, [rip + syscall_entry];" // RAX = address of syscall_entry
                                        // function
