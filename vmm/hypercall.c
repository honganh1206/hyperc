...
typedef struct fd_handle {
  int real_fd;
  // Flag indicating whether this mapping slot is used
  int opening;
} fd_handle;
...
// Get VM output
// Quick way to convert data (reduce boilerplate)
...
// Check out of bounds
#define CHECK_OOB(offset) assert(0 <= (offset) && (offset) < vm->mem_size)

/*
When this expands to C code, a vm param is added:

 static inline uint8_t* mem_at(VM *vm, uint64_t offset) {
  assert(0 <= offset && offset < vm->mem_size);
  return (uint8_t*) vm->mem + (uint64_t) offset;
 }
*/
#define MEM_AT(offset)                                                         \
  (CHECK_OOB(offset), (uint8_t *)vm->mem + (uint64_t)offset)
...
static int hp_handle_open(VM *vm) {
  ...
  PROCESS {
    ...
    if (min_fd > MAX_FD)
      ...
      // Guest kernel makes read hypercall to VMM
      // then VMM makes syscall to host kernel lol
...
static int handle_rw(VM *vm, typeof(read) fptr) {
  ...
  PROCESS {
    ...
    // Check bound
    if (!(0 <= paddr && paddr <= paddr + nbytes &&
          paddr + nbytes <= vm->mem_size))
      ret = -EACCES;
    else
...
static int hp_handle_read(VM *vm) { return handle_rw(vm, read); }

static int hp_handle_write(VM *vm) {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wincompatible-pointer-types"
  // Cast write function pointer to read
  // but casting like this could prove incompatible
  // so we silence the warnings instead
